/****************************************************************************
**
** RFIDScannerCP.h -- RFIDScanner connection point proxy, to actually fire the events.
**
...
#ifndef __RFIDSCANNERCP_H__
#define __RFIDSCANNERCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSRFIDScanner;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSRFIDScannerEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSRFIDScanner* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDError = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSRFIDScannerEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSRFIDScannerEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSRFIDScanner* _pOposControl );
};

#endif //__RFIDSCANNERCP_H__
