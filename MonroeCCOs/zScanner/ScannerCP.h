/****************************************************************************
**
** ScannerCP.h -- Scanner connection point proxy, to actually fire the events.
**
...
#ifndef __SCANNERCP_H__
#define __SCANNERCP_H__
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSScannerEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSScanner* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDError = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSScannerEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSScannerEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSScanner* _pOposControl );
};

#endif //__SCANNERCP_H__
