/****************************************************************************
**
** LineDisplayCP.h -- LineDisplay connection point proxy, to actually fire the events.
**
...
#ifndef __LINEDISPLAYCP_H__
#define __LINEDISPLAYCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSLineDisplay;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSLineDisplayEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSLineDisplay* pOposControl ) )
    {
        _DIDDirectIO = -1;
        _DIDStatus = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSLineDisplayEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSLineDisplayEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSLineDisplay* _pOposControl );
};

#endif //__LINEDISPLAYCP_H__
