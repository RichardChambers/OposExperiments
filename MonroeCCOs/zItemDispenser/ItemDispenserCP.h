/****************************************************************************
**
** ItemDispenserCP.h -- ItemDispenser connection point proxy, to actually fire the events.
**
...
#ifndef __ITEMDISPENSERCP_H__
#define __ITEMDISPENSERCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSItemDispenser;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSItemDispenserEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSItemDispenser* pOposControl ) )
    {
        _DIDDirectIO = -1;
        _DIDStatus = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSItemDispenserEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSItemDispenserEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSItemDispenser* _pOposControl );
};

#endif //__ITEMDISPENSERCP_H__
