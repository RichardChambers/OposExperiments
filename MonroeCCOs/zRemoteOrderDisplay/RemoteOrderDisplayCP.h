/****************************************************************************
**
** RemoteOrderDisplayCP.h -- RemoteOrderDisplay connection point proxy, to actually fire the events.
**
...
#ifndef __REMOTEORDERDISPLAYCP_H__
#define __REMOTEORDERDISPLAYCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSRemoteOrderDisplay;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSRemoteOrderDisplayEvents, CComDynamicUnkArray>
{
public:
...
//===========================================================================
    ...
    VOID Fire_OutputCompleteEvent(LONG OutputID)
    {
        T* pT = static_cast<T*>(this);
        OposVariant var;
        int nConnections = m_vec.GetSize();
        ...
        for (_nCOConnectionIndex = 0; _nCOConnectionIndex < nConnections; _nCOConnectionIndex++)
        {
            ...
            if (_pCPDispatch != NULL)
            {
                var.SetLONG( OutputID );
                FireTheEvent( FEUNKNOWN &var, 1, _DIDOutput, L"OutputCompleteEvent", 4 );
            }
        }
    }
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSRemoteOrderDisplay* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDError = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSRemoteOrderDisplayEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSRemoteOrderDisplayEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSRemoteOrderDisplay* _pOposControl );
};

#endif //__REMOTEORDERDISPLAYCP_H__
