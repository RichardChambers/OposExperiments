/****************************************************************************
**
** CoinDispenserCP.h -- CoinDispenser connection point proxy, to actually fire the events.
**
...
#ifndef __COINDISPENSERCP_H__
#define __COINDISPENSERCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSCoinDispenser;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSCoinDispenserEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSCoinDispenser* pOposControl ) )
    {
        _DIDDirectIO = -1;
        _DIDStatus = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSCoinDispenserEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSCoinDispenserEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSCoinDispenser* _pOposControl );
};

#endif //__COINDISPENSERCP_H__
