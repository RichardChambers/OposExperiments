/****************************************************************************
**
** SmartCardRWCP.h -- SmartCardRW connection point proxy, to actually fire the events.
**
...
#ifndef __SMARTCARDRWCP_H__
#define __SMARTCARDRWCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSSmartCardRW;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSSmartCardRWEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSSmartCardRW* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDError = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSSmartCardRWEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSSmartCardRWEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSSmartCardRW* _pOposControl );
};

#endif //__SMARTCARDRWCP_H__
