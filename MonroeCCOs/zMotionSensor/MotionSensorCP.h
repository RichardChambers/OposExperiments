/****************************************************************************
**
** MotionSensorCP.h -- MotionSensor connection point proxy, to actually fire the events.
**
...
#ifndef __MOTIONSENSORCP_H__
#define __MOTIONSENSORCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSMotionSensor;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSMotionSensorEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSMotionSensor* pOposControl ) )
    {
        _DIDDirectIO = -1;
        _DIDStatus = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSMotionSensorEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSMotionSensorEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSMotionSensor* _pOposControl );
};

#endif //__MOTIONSENSORCP_H__
