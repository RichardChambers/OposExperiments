/****************************************************************************
**
** CashChangerCP.h -- CashChanger connection point proxy, to actually fire the events.
**
...
#ifndef __CASHCHANGERCP_H__
#define __CASHCHANGERCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSCashChanger;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSCashChangerEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSCashChanger* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDStatus = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSCashChangerEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSCashChangerEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSCashChanger* _pOposControl );
};

#endif //__CASHCHANGERCP_H__
