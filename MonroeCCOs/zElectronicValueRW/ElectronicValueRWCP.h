/****************************************************************************
**
** ElectronicValueRWCP.h -- ElectronicValueRW connection point proxy, to actually fire the events.
**
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSElectronicValueRWEvents, CComDynamicUnkArray>
{
public:
...
//===========================================================================
    ...
    VOID Fire_TransitionEvent(LONG EventNumber, LONG* pData, BSTR* pString)
    {
        T* pT = static_cast<T*>(this);
        OposVariant vars[3];
        int nConnections = m_vec.GetSize();
        ...
        for (_nCOConnectionIndex = 0; _nCOConnectionIndex < nConnections; _nCOConnectionIndex++)
        {
            ...
            if (_pCPDispatch != NULL)
            {
                ...
                vars[1].SetLONG_Ptr( pData, hr );
                vars[0].SetBSTR_InOutPtr( pString, hr );
                FireTheEvent( FEUNKNOWN vars, 3, _DIDTransition, L"TransitionEvent", 6 );
            }
        }
    }
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSElectronicValueRW* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDError = -1;
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSElectronicValueRWEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
