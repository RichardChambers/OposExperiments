/****************************************************************************
**
** ElectronicValueRWEvent.h -- OPOS event firing support.
**
**    This file is included into the ElectronicValueRW Control Object's main
...
#ifndef __ELECTRONICVALUERWEVENT_H__
#define __ELECTRONICVALUERWEVENT_H__
...
/////////////////////////////////////////////////////////////////////////////
    ...
    // Temporary holding places for Error params that won't fit in msg params.
    long    _Event_Err_ErrorLocus;
    long*   _Event_Err_pErrorResponse;
    // Temporary holding place for Transition param that won't fit in msg params.
    BSTR*   _Event_Transition_pString;
...
/////////////////////////////////////////////////////////////////////////////
// Function name    : EventClaim
// Description      : Called from the control object's Claim method
//                      (only for exclusive use devices).
// Return type      : void
/////////////////////////////////////////////////////////////////////////////

    void EventClaim()
    {
        DOTRACEV( ( _T("*EventClaim") ) );

        // Initialize so that events are allowed.
        //   (Do in "Claim" in case a call to "Release" disabled events.
        //    We now need to reenable.)
        _Event_bEventsLegal = true;
    }

/////////////////////////////////////////////////////////////////////////////
// Function name    : EventPreCloseRelease
// Description      : Called from the control object's Release and Close methods.
// Return type      : void
/////////////////////////////////////////////////////////////////////////////

    void EventPreCloseRelease()
    {
        ...
        //   (This can only occur if the Close or Release is called from a thread
...
/////////////////////////////////////////////////////////////////////////////
    ...
    STDMETHOD( SOData )(long Status)
    {
        DOTRACEV( ( _T("+DataEvent: Status %d (0x%X)"), Status, Status ) );

        if ( _Event_ControlThreadID == GetCurrentThreadId() )
            CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_DataEvent( Status );
        else
            DoEvent( WMU_DATA_EVENT, Status, 0 );

        DOTRACEV( ( _T("-DataEvent") ) );
        return S_OK;
    }
...
//===========================================================================
    ...
    STDMETHOD( SODirectIO )(long EventNumber, long* pData, BSTR* pString)
    {
        ...
        if ( _Event_ControlThreadID == GetCurrentThreadId() )
            CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_DirectIOEvent( EventNumber, pData, pString );
        else
        {
...
//===========================================================================
    ...
    STDMETHOD( SOError )(long ResultCode, long ResultCodeExtended,
                long ErrorLocus, long* pErrorResponse)
    {
        DOTRACEV( ( _T("+ErrorEvent: RC %d, RCExt %d (0x%X), Locus %d, Response %d"), \
            ResultCode, ResultCodeExtended, ResultCodeExtended, ErrorLocus, *pErrorResponse ) );

        if ( _Event_ControlThreadID == GetCurrentThreadId() )
            CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_ErrorEvent( ResultCode, ResultCodeExtended,
                    ErrorLocus, pErrorResponse );
        else
        {
            _Event_Err_ErrorLocus = ErrorLocus;
            _Event_Err_pErrorResponse = pErrorResponse;
            DoEvent( WMU_ERROR_EVENT, ResultCode, ResultCodeExtended );
        }

        DOTRACEV( ( _T("-ErrorEvent: Response %d"), *pErrorResponse ) );
        return S_OK;
    }

//===========================================================================

    STDMETHOD( SOOutputComplete )(long OutputID)
    {
        DOTRACEV( ( _T("+OutputCompleteEvent: OutputID %d"), OutputID ) );

        if ( _Event_ControlThreadID == GetCurrentThreadId() )
            CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_OutputCompleteEvent( OutputID );
        else
            DoEvent( WMU_OUTPUT_COMPLETE_EVENT, OutputID, 0 );

        DOTRACEV( ( _T("-OutputCompleteEvent") ) );
        return S_OK;
    }
...
//===========================================================================
    ...
    STDMETHOD( SOStatusUpdate)(long Data)
    {
        ...
        if ( _Event_ControlThreadID == GetCurrentThreadId() )
            CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_StatusUpdateEvent( Data );
        else
...
//===========================================================================
    ...
    STDMETHOD( SOTransition )(long EventNumber, long* pData, BSTR* pString)
    {
        ...
        DOTRACESTRINGV( _T("    String."), *pString );
        HRESULT hr;
        OposVariant TransStr;
        TransStr.SetBSTR_Copy( *pString, hr );
    #endif

        if ( _Event_ControlThreadID == GetCurrentThreadId() )
            CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_TransitionEvent( EventNumber, pData, pString );
        else
        {
            _Event_Transition_pString = pString;
        ...
        if ( nStrLen == ::SysStringByteLen( *pString ) &&
            ...
            DOTRACEV( ( _T("-TransitionEvent: Data %d (0x%X); String Unchanged"), *pData, *pData ) );
        else
        {
            DOTRACEV( ( _T("-TransitionEvent: Data %d (0x%X)"), *pData, *pData ) );
            DOTRACESTRINGV( _T("    String."), *pString );
        }
    #endif
        return S_OK;
...
/////////////////////////////////////////////////////////////////////////////
    ...
    {
        COPOSElectronicValueRW* pThis;

        // On window creation, save our window instance into user window space.
        if ( uMsg == WM_CREATE )
        {
            pThis = (COPOSElectronicValueRW*) ((LPCREATESTRUCT) lParam)->lpCreateParams;
        ...
        {
            pThis = (COPOSElectronicValueRW*) ::GetWindowLong( hwnd, 0 );
            if ( pThis != 0 &&
...
/////////////////////////////////////////////////////////////////////////////
        ...
        UINT uMsg, WPARAM wParam, LPARAM lParam )
    {
        ...
        case WMU_DATA_EVENT:
            {
                if ( FireEventSetup() )
                {
                    DOTRACEV( ( _T("*EventWindowProc: Firing DataEvent") ) );
                    CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_DataEvent(
                        (long) wParam );
                    FireEventCleanup();
                }
                break;
        ...
        case WMU_DIRECT_IO_EVENT:
            {
                if ( FireEventSetup() )
                {
                    ...
                    CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_DirectIOEvent(
        ...
        case WMU_ERROR_EVENT:
            {
                if ( FireEventSetup() )
                {
                    DOTRACEV( ( _T("*EventWindowProc: Firing ErrorEvent") ) );
                    CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_ErrorEvent (
                        (long) wParam, (long) lParam, _Event_Err_ErrorLocus, _Event_Err_pErrorResponse );
                    FireEventCleanup();
                }
                break;
            }

        case WMU_OUTPUT_COMPLETE_EVENT:
            {
                if ( FireEventSetup() )
                {
                    DOTRACEV( ( _T("*EventWindowProc: Firing OutputCompleteEvent") ) );
                    CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_OutputCompleteEvent(
                        (long) wParam );
                    FireEventCleanup();
                }
                break;
        ...
        case WMU_STATUS_UPDATE_EVENT:
            {
                if ( FireEventSetup() )
                {
                    ...
                    CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_StatusUpdateEvent(
                        (long) wParam );
                    FireEventCleanup();
                }
                break;
        ...
        case WMU_TRANSITION_EVENT:
            {
                if ( FireEventSetup() )
                {
                    DOTRACEV( ( _T("*EventWindowProc: Firing TransitionEvent") ) );
                    CProxy_IOPOSEvents< COPOSElectronicValueRW >::Fire_TransitionEvent(
                        (long) wParam, (long*) lParam, _Event_Transition_pString );
                    FireEventCleanup();
                }
                break;
            }
...
#endif //__ELECTRONICVALUERWEVENT_H__

// end ElectronicValueRWEvent.h
