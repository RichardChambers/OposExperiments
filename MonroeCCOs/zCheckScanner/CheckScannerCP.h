/****************************************************************************
**
** CheckScannerCP.h -- CheckScanner connection point proxy, to actually fire the events.
**
...
#ifndef __CHECKSCANNERCP_H__
#define __CHECKSCANNERCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSCheckScanner;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSCheckScannerEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSCheckScanner* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDError = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSCheckScannerEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSCheckScannerEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSCheckScanner* _pOposControl );
};

#endif //__CHECKSCANNERCP_H__
