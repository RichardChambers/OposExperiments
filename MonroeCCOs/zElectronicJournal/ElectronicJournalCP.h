/****************************************************************************
**
** ElectronicJournalCP.h -- ElectronicJournal connection point proxy, to actually fire the events.
**
...
#ifndef __ELECTRONICJOURNALCP_H__
#define __ELECTRONICJOURNALCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSElectronicJournal;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSElectronicJournalEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSElectronicJournal* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDError = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSElectronicJournalEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSElectronicJournalEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSElectronicJournal* _pOposControl );
};

#endif //__ELECTRONICJOURNALCP_H__
