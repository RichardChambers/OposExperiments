/****************************************************************************
**
** BillDispenserCP.h -- BillDispenser connection point proxy, to actually fire the events.
**
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSBillDispenserEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSBillDispenser* pOposControl ) )
    {
        _DIDDirectIO = -1;
        _DIDStatus = -1;
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSBillDispenserEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSBillDispenser* _pOposControl );
};

#endif //__BILLDISPENSERCP_H__
