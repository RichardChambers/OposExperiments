/****************************************************************************
**
** KeylockCP.h -- Keylock connection point proxy, to actually fire the events.
**
...
#ifndef __KEYLOCKCP_H__
#define __KEYLOCKCP_H__
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSKeylockEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSKeylock* pOposControl ) )
    {
        _DIDDirectIO = -1;
        _DIDStatus = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSKeylockEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSKeylockEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSKeylock* _pOposControl );
};

#endif //__KEYLOCKCP_H__
