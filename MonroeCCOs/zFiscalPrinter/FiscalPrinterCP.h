/****************************************************************************
**
** FiscalPrinterCP.h -- FiscalPrinter connection point proxy, to actually fire the events.
**
...
#ifndef __FISCALPRINTERCP_H__
#define __FISCALPRINTERCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSFiscalPrinter;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSFiscalPrinterEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSFiscalPrinter* pOposControl ) )
    {
        _DIDDirectIO = -1;
        _DIDError = -1;
        _DIDOutput = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSFiscalPrinterEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSFiscalPrinterEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSFiscalPrinter* _pOposControl );
};

#endif //__FISCALPRINTERCP_H__
