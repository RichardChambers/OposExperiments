/****************************************************************************
**
** CoinAcceptorCP.h -- CoinAcceptor connection point proxy, to actually fire the events.
**
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSCoinAcceptorEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSCoinAcceptor* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDStatus = -1;
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSCoinAcceptorEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSCoinAcceptor* _pOposControl );
};

#endif //__COINACCEPTORCP_H__
