/****************************************************************************
**
** CATCP.h -- CAT connection point proxy, to actually fire the events.
**
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSCATEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSCAT* pOposControl ) )
    {
        _DIDDirectIO = -1;
        _DIDError = -1;
        _DIDOutput = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSCATEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSCATEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSCAT* _pOposControl );
};
