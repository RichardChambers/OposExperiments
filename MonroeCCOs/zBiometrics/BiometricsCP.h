/****************************************************************************
**
** BiometricsCP.h -- Biometrics connection point proxy, to actually fire the events.
**
...
#ifndef __BIOMETRICSCP_H__
#define __BIOMETRICSCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSBiometrics;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSBiometricsEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSBiometrics* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDError = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSBiometricsEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSBiometricsEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSBiometrics* _pOposControl );
};

#endif //__BIOMETRICSCP_H__
