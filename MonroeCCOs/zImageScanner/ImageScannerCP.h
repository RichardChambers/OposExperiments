/****************************************************************************
**
** ImageScannerCP.h -- ImageScanner connection point proxy, to actually fire the events.
**
...
#ifndef __IMAGESCANNERCP_H__
#define __IMAGESCANNERCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSImageScanner;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSImageScannerEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSImageScanner* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDError = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSImageScannerEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSImageScannerEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSImageScanner* _pOposControl );
};

#endif //__IMAGESCANNERCP_H__
