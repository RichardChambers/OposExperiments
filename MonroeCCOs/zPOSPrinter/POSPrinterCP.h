/****************************************************************************
**
** POSPrinterCP.h -- POSPrinter connection point proxy, to actually fire the events.
**
...
#ifndef __POSPRINTERCP_H__
#define __POSPRINTERCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSPOSPrinter;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSPOSPrinterEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSPOSPrinter* pOposControl ) )
    {
        _DIDDirectIO = -1;
        _DIDError = -1;
        _DIDOutput = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSPOSPrinterEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSPOSPrinterEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSPOSPrinter* _pOposControl );
};

#endif //__POSPRINTERCP_H__
