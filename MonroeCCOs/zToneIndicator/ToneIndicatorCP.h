/****************************************************************************
**
** ToneIndicatorCP.h -- ToneIndicator connection point proxy, to actually fire the events.
**
...
#ifndef __TONEINDICATORCP_H__
#define __TONEINDICATORCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSToneIndicator;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSToneIndicatorEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSToneIndicator* pOposControl ) )
    {
        _DIDDirectIO = -1;
        _DIDError = -1;
        _DIDOutput = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSToneIndicatorEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSToneIndicatorEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSToneIndicator* _pOposControl );
};

#endif //__TONEINDICATORCP_H__
