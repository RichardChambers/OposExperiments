...
TCHAR g_WindowName[] = _T("HW_OPOSTotals");

// Control Object description and version.
char g_ControlDescription[] = "OPOS Totals Control 1.14.001 [Public, by CRM/MCS]" IFDEBUG(" [With Tracing]");
...
int COPOSTotals::s_nInstances = 0;
...
// Release 1.0 methods
    ...
    "BeginTrans",
        #define nDIBeginTrans 11
    "ClaimFile",
        #define nDIClaimFile 12
    "CommitTrans",
        #define nDICommitTrans 13
    "Create",
        #define nDICreate 14
    "Delete",
        #define nDIDelete 15
    "Find",
        #define nDIFind 16
    "FindByIndex",
        #define nDIFindByIndex 17
    "Read",
        #define nDIRead 18
    "RecalculateValidationData",
        #define nDIRecalculateValidationData 19
    "ReleaseFile",
        #define nDIReleaseFile 20
    "Rename",
        #define nDIRename 21
    "Rollback",
        #define nDIRollback 22
    "SetAll",
        #define nDISetAll 23
    "ValidateData",
        #define nDIValidateData 24
    "Write",
        #define nDIWrite 25
// Release 1.1 methods
// Release 1.2 methods
...
// Release 1.8 methods
    "ResetStatistics",
        #define nDIResetStatistics 26
    "RetrieveStatistics",
        #define nDIRetrieveStatistics 27
    "UpdateStatistics",
        #define nDIUpdateStatistics 28
// Release 1.9 methods
    "CompareFirmwareVersion",
        #define nDICompareFirmwareVersion 29
    "UpdateFirmware",
        #define nDIUpdateFirmware 30
    0
};
...
//   for each release.
// Followed by the initial minor release of the OPOS Totals.
//===========================================================================
...
COPOSTotals::COPOSTotals()
    IFDEBUG( : CProxy_IOPOSEvents< COPOSTotals >(this) )
{
}

//===========================================================================

HRESULT COPOSTotals::FinalConstruct()
{
    TRACEINIT;
    DOTRACE( ( _T("") ) );
...
STDMETHODIMP COPOSTotals::FreezeEvents( BOOL bFreeze )
{
...
HRESULT COPOSTotals::OnDraw( ATL_DRAWINFO& di )
{
    ...
    HBITMAP hBitmap = LoadBitmap(
        ...
        MAKEINTRESOURCE(IDB_OPOSTOTALS2) ); // bitmap resource name
    BITMAP bmp;
    GetObject(
...
HRESULT COPOSTotals::GetOposProp( DEBUGPARAM(LPCSTR pPropName)
...
HRESULT COPOSTotals::SetOposProp( DEBUGPARAM(LPCSTR pPropName)
...
HRESULT COPOSTotals::DoInvoke( DEBUGPARAM(LPCSTR pFuncName)
...
void COPOSTotals::SetCOFreezeEvents( VARIANT_BOOL bFreeze )
{
    // If open...
    if ( _bOpened )
    {
...
STDMETHODIMP COPOSTotals::Open( BSTR DeviceName, long *pRC )
{
    HRESULT hRC;
    LONG nError;
    ...
    *pRC = SetRC( OPOS_E_NOEXIST );
    ...
    HKEY hBaseKey;
    cw_strcpy( szBaseKey, OPOS_ROOTKEY "\\" OPOS_CLASSKEY_TOT );
    nError = RegOpenKeyEx(
    ...
    *pRC = SetRC( OPOS_E_NOSERVICE );
    ...
    OposVariant Vars[3];
    Vars[2].SetString( _CW(OPOS_CLASSKEY_TOT), hRC );
...
STDMETHODIMP COPOSTotals::Close( long *pRC )
{
    SetRC();
...
STDMETHODIMP COPOSTotals::ClaimDevice( long Timeout, long *pRC )
{
    SetRC();
...
STDMETHODIMP COPOSTotals::ReleaseDevice( long *pRC )
{
    SetRC();
...
STDMETHODIMP COPOSTotals::SOProcessID( long *pProcessID )
{
...
STDMETHODIMP COPOSTotals::CheckHealth( 
    /*[in]*/ LONG Level, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Var;
...
STDMETHODIMP COPOSTotals::DirectIO( 
    /*[in]*/ LONG Command, 
    /*[in, out]*/ LONG* pData, 
    /*[in, out]*/ BSTR* pString, 
    /*[out, retval]*/ long* pRC )
...
STDMETHODIMP COPOSTotals::BeginTrans( 
    /*[out, retval]*/ long* pRC )
{
    return DoInvoke( DEBUGPARAM("BeginTrans") S_OK, NULL, 0, nDIBeginTrans, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::ClaimFile( 
    /*[in]*/ LONG HTotalsFile, 
    /*[in]*/ LONG Timeout, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Vars[2];
    Vars[1].SetLONG( HTotalsFile );
    Vars[0].SetLONG( Timeout );
    return DoInvoke( DEBUGPARAM("ClaimFile") S_OK, Vars, 2, nDIClaimFile, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::CommitTrans( 
    /*[out, retval]*/ long* pRC )
{
    return DoInvoke( DEBUGPARAM("CommitTrans") S_OK, NULL, 0, nDICommitTrans, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::Create( 
    /*[in]*/ BSTR FileName, 
    ...
    /*[out, retval]*/ long* pRC )
{
    ...
    Vars[2].SetLONG_Ptr( pHTotalsFile, hRC );
    Vars[1].SetLONG( Size );
    Vars[0].SetVARIANT_BOOL( ErrorDetection );
    return DoInvoke( DEBUGPARAM("Create") hRC, Vars, 4, nDICreate, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::Delete( 
    /*[in]*/ BSTR FileName, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Var;
    ...
    return DoInvoke( DEBUGPARAM("Delete") hRC, &Var, 1, nDIDelete, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::Find( 
    /*[in]*/ BSTR FileName, 
    /*[out]*/ LONG* pHTotalsFile, 
    /*[out]*/ LONG* pSize, 
    /*[out, retval]*/ long* pRC )
{
    ...
    Vars[1].SetLONG_Ptr( pHTotalsFile, hRC );
    Vars[0].SetLONG_Ptr( pSize, hRC );
    return DoInvoke( DEBUGPARAM("Find") hRC, Vars, 3, nDIFind, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::FindByIndex( 
    /*[in]*/ LONG Index, 
    /*[out]*/ BSTR* pFileName, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Vars[2];
    HRESULT hRC = S_OK;
    Vars[1].SetLONG( Index );
    Vars[0].SetBSTR_OutPtr( pFileName, hRC );
    return DoInvoke( DEBUGPARAM("FindByIndex") hRC, Vars, 2, nDIFindByIndex, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::Read( 
    /*[in]*/ LONG HTotalsFile, 
    /*[out]*/ BSTR* pData, 
    /*[in]*/ LONG Offset, 
    /*[in]*/ LONG Count, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Vars[4];
    HRESULT hRC = S_OK;
    Vars[3].SetLONG( HTotalsFile );
    Vars[2].SetBSTR_OutPtr( pData, hRC );
    Vars[1].SetLONG( Offset );
    Vars[0].SetLONG( Count );
    return DoInvoke( DEBUGPARAM("Read") hRC, Vars, 4, nDIRead, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::RecalculateValidationData( 
    /*[in]*/ LONG HTotalsFile, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Var;
    Var.SetLONG( HTotalsFile );
    return DoInvoke( DEBUGPARAM("RecalculateValidationData") S_OK, &Var, 1, nDIRecalculateValidationData, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::ReleaseFile( 
    /*[in]*/ LONG HTotalsFile, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Var;
    Var.SetLONG( HTotalsFile );
    return DoInvoke( DEBUGPARAM("ReleaseFile") S_OK, &Var, 1, nDIReleaseFile, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::Rename( 
    /*[in]*/ LONG HTotalsFile, 
    /*[in]*/ BSTR FileName, 
    /*[out, retval]*/ long* pRC )
{
    ...
    HRESULT hRC = S_OK;
    Vars[1].SetLONG( HTotalsFile );
    Vars[0].SetBSTR( FileName, hRC );
    return DoInvoke( DEBUGPARAM("Rename") hRC, Vars, 2, nDIRename, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::Rollback( 
    /*[out, retval]*/ long* pRC )
{
    return DoInvoke( DEBUGPARAM("Rollback") S_OK, NULL, 0, nDIRollback, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::SetAll( 
    /*[in]*/ LONG HTotalsFile, 
    /*[in]*/ LONG Value, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Vars[2];
    Vars[1].SetLONG( HTotalsFile );
    Vars[0].SetLONG( Value );
    return DoInvoke( DEBUGPARAM("SetAll") S_OK, Vars, 2, nDISetAll, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::ValidateData( 
    /*[in]*/ LONG HTotalsFile, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Var;
    Var.SetLONG( HTotalsFile );
    return DoInvoke( DEBUGPARAM("ValidateData") S_OK, &Var, 1, nDIValidateData, pRC );
}

//===========================================================================

STDMETHODIMP COPOSTotals::Write( 
    /*[in]*/ LONG HTotalsFile, 
    /*[in]*/ BSTR Data, 
    /*[in]*/ LONG Offset, 
    /*[in]*/ LONG Count, 
    /*[out, retval]*/ long* pRC )
{
    ...
    HRESULT hRC = S_OK;
    Vars[3].SetLONG( HTotalsFile );
    Vars[2].SetBSTR( Data, hRC );
    Vars[1].SetLONG( Offset );
    Vars[0].SetLONG( Count );
    return DoInvoke( DEBUGPARAM("Write") hRC, Vars, 4, nDIWrite, pRC );
}
...
STDMETHODIMP COPOSTotals::ResetStatistics( 
...
STDMETHODIMP COPOSTotals::RetrieveStatistics( 
...
STDMETHODIMP COPOSTotals::UpdateStatistics( 
    /*[in]*/ BSTR StatisticsBuffer, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Var;
...
STDMETHODIMP COPOSTotals::CompareFirmwareVersion( 
...
STDMETHODIMP COPOSTotals::UpdateFirmware( 
    /*[in]*/ BSTR FirmwareFileName, 
    /*[out, retval]*/ long* pRC )
{
    OposVariant Var;
...
STDMETHODIMP COPOSTotals::get_FreezeEvents( VARIANT_BOOL *pFreezeEvents )
{
...
STDMETHODIMP COPOSTotals::put_FreezeEvents( VARIANT_BOOL FreezeEvents )
{
    SetRC();
...
STDMETHODIMP COPOSTotals::get_ResultCode( long *pResultCode )
{
...
STDMETHODIMP COPOSTotals::get_State( long *pState )
{
    if ( ! _bOpened )                           // If not open,
    {
...
STDMETHODIMP COPOSTotals::get_ControlObjectDescription( BSTR *pControlObjectDescription )
{
...
STDMETHODIMP COPOSTotals::get_ControlObjectVersion( long *pControlObjectVersion )
{
...
STDMETHODIMP COPOSTotals::get_OpenResult( long *pOpenResult )
{
...
STDMETHODIMP COPOSTotals::get_CheckHealthText( /*[out, retval]*/ BSTR* pCheckHealthText )
{
...
STDMETHODIMP COPOSTotals::get_Claimed( /*[out, retval]*/ VARIANT_BOOL* pClaimed )
{
...
STDMETHODIMP COPOSTotals::get_DeviceEnabled( /*[out, retval]*/ VARIANT_BOOL* pDeviceEnabled )
{
...
STDMETHODIMP COPOSTotals::put_DeviceEnabled( /*[in]*/ VARIANT_BOOL DeviceEnabled )
{
...
STDMETHODIMP COPOSTotals::get_ResultCodeExtended( /*[out, retval]*/ LONG* pResultCodeExtended )
{
...
STDMETHODIMP COPOSTotals::get_ServiceObjectDescription( /*[out, retval]*/ BSTR* pServiceObjectDescription )
{
...
STDMETHODIMP COPOSTotals::get_ServiceObjectVersion( /*[out, retval]*/ LONG* pServiceObjectVersion )
{
...
STDMETHODIMP COPOSTotals::get_DeviceDescription( /*[out, retval]*/ BSTR* pDeviceDescription )
{
...
STDMETHODIMP COPOSTotals::get_DeviceName( /*[out, retval]*/ BSTR* pDeviceName )
{
...
STDMETHODIMP COPOSTotals::get_CapErrorDetection( /*[out, retval]*/ VARIANT_BOOL* pCapErrorDetection )
{
    return GetOposProp( DEBUGPARAM("CapErrorDetection") PIDXTot_CapErrorDetection, pCapErrorDetection, 0 /*MinorVersion*/ );
}

//===========================================================================

STDMETHODIMP COPOSTotals::get_CapSingleFile( /*[out, retval]*/ VARIANT_BOOL* pCapSingleFile )
{
    return GetOposProp( DEBUGPARAM("CapSingleFile") PIDXTot_CapSingleFile, pCapSingleFile, 0 /*MinorVersion*/ );
}

//===========================================================================

STDMETHODIMP COPOSTotals::get_CapTransactions( /*[out, retval]*/ VARIANT_BOOL* pCapTransactions )
{
    return GetOposProp( DEBUGPARAM("CapTransactions") PIDXTot_CapTransactions, pCapTransactions, 0 /*MinorVersion*/ );
}

//===========================================================================

STDMETHODIMP COPOSTotals::get_FreeData( /*[out, retval]*/ LONG* pFreeData )
{
    return GetOposProp( DEBUGPARAM("FreeData") PIDXTot_FreeData, pFreeData, 0 /*MinorVersion*/ );
}

//===========================================================================

STDMETHODIMP COPOSTotals::get_NumberOfFiles( /*[out, retval]*/ LONG* pNumberOfFiles )
{
    return GetOposProp( DEBUGPARAM("NumberOfFiles") PIDXTot_NumberOfFiles, pNumberOfFiles, 0 /*MinorVersion*/ );
}

//===========================================================================

STDMETHODIMP COPOSTotals::get_TotalsSize( /*[out, retval]*/ LONG* pTotalsSize )
{
    return GetOposProp( DEBUGPARAM("TotalsSize") PIDXTot_TotalsSize, pTotalsSize, 0 /*MinorVersion*/ );
}

//===========================================================================

STDMETHODIMP COPOSTotals::get_TransactionInProgress( /*[out, retval]*/ VARIANT_BOOL* pTransactionInProgress )
{
    return GetOposProp( DEBUGPARAM("TransactionInProgress") PIDXTot_TransactionInProgress, pTransactionInProgress, 0 /*MinorVersion*/ );
}
...
STDMETHODIMP COPOSTotals::get_BinaryConversion( /*[out, retval]*/ LONG* pBinaryConversion )
{
...
STDMETHODIMP COPOSTotals::put_BinaryConversion( /*[in]*/ LONG BinaryConversion )
{
...
STDMETHODIMP COPOSTotals::get_CapPowerReporting( /*[out, retval]*/ LONG* pCapPowerReporting )
{
...
STDMETHODIMP COPOSTotals::get_PowerNotify( /*[out, retval]*/ LONG* pPowerNotify )
{
...
STDMETHODIMP COPOSTotals::put_PowerNotify( /*[in]*/ LONG PowerNotify )
{
...
STDMETHODIMP COPOSTotals::get_PowerState( /*[out, retval]*/ LONG* pPowerState )
{
...
STDMETHODIMP COPOSTotals::get_CapStatisticsReporting( /*[out, retval]*/ VARIANT_BOOL* pCapStatisticsReporting )
{
...
STDMETHODIMP COPOSTotals::get_CapUpdateStatistics( /*[out, retval]*/ VARIANT_BOOL* pCapUpdateStatistics )
{
...
STDMETHODIMP COPOSTotals::get_CapCompareFirmwareVersion( /*[out, retval]*/ VARIANT_BOOL* pCapCompareFirmwareVersion )
{
...
STDMETHODIMP COPOSTotals::get_CapUpdateFirmware( /*[out, retval]*/ VARIANT_BOOL* pCapUpdateFirmware )
{
...
void COPOSTotals::WriteTrace( LPCTSTR pFormat, ... )
{
    static TCHAR s_TraceFile[] = _T("C:\\OposCCO_Totals.log");
...
#define OPOSCCOCLASS COPOSTotals
#define RCSTRACENAME "OposCCO_Totals"
...
void COPOSTotals::WriteTraceError( LPCTSTR pFormat, DWORD dwError )
{
    if ( dwError != 0 )
    {
...
void COPOSTotals::WriteTraceMethods( DISPID* pDispID, int nMethods )
{
...
//## Output string.
void COPOSTotals::WriteTraceString( LPCTSTR pszPrefix, BSTR bstr )
{
    // Get string length.
...
void COPOSTotals::WriteTraceString( void* pString, int nLen, BOOL bAscii, int nCharSize,
    LPCTSTR pszFormat, ... )
{
