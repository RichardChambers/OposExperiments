/****************************************************************************
**
** BillAcceptorCP.h -- BillAcceptor connection point proxy, to actually fire the events.
**
...
#ifndef __BILLACCEPTORCP_H__
#define __BILLACCEPTORCP_H__

#if defined(_DEBUG) && defined(WRITETRACEERROR)
    class COPOSBillAcceptor;
...
template <class T>
class CProxy_IOPOSEvents : public IConnectionPointImpl<T, &DIID__IOPOSBillAcceptorEvents, CComDynamicUnkArray>
{
public:
...
// Constructor: Initialize cached dispatch IDs.
    CProxy_IOPOSEvents( IFDEBUG( COPOSBillAcceptor* pOposControl ) )
    {
        _DIDData = -1;
        _DIDDirectIO = -1;
        _DIDStatus = -1;
    ...
    {
        HRESULT hr = IConnectionPointImpl<T, &DIID__IOPOSBillAcceptorEvents, CComDynamicUnkArray>::Advise( pUnkSink, pdwCookie );
    ...
    STDMETHOD(Unadvise)(DWORD dwCookie)
    {
        ...
        return IConnectionPointImpl<T, &DIID__IOPOSBillAcceptorEvents, CComDynamicUnkArray>::Unadvise(dwCookie);
    }
#endif
...
//   so that this data can be class level.
    ...
    IFDEBUG( COPOSBillAcceptor* _pOposControl );
};

#endif //__BILLACCEPTORCP_H__
